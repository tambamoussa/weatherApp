...
const API_KEY = "f7666b7c36e24f579e930844241608";
...
const weatherCodes = {
  ...
  mist: [1030, 1135, 1147],
  rain: [1063, 1150, 1153, 1168, 1180, 1183, 1198, 1201, 1240, 1243, 1246, 1273, 1276],
...
//Generate HTML for each hourly forecast and display it
  ...
  const weatherIcon = Object.keys(weatherCodes).find(icon => weatherCodes[icon]
    .includes(item.condition.code));
...
try {
  //Fetch wearher data from the API and parse the response as JSON
...
//Extract current weather details
  ...
  const weatherIcon = Object.keys(weatherCodes).find(icon => weatherCodes[icon]
    .includes(data.current.condition.code));
...
//Set up the wearher request for a specific city
...
searchInput.addEventListener("keyup", (e) => {
   const cityName = searchInput.value.trim();  // j'utilise 'value' pour récupérer la valeur saisie
